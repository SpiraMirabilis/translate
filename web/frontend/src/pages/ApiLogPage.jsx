...
export default function ApiLogPage() {
  ...
  const totalTokens = sessions.reduce((s, sess) =>
    s + sess.calls.reduce((t, c) => t + (c.total_tokens || c.completion_tokens || 0), 0), 0)
  const totalFailures = sessions.reduce((s, sess) =>
    s + sess.calls.filter(c => !c.success).length, 0)
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400">
            {totalCalls} call{totalCalls !== 1 ? 's' : ''} across {sessions.length} session{sessions.length !== 1 ? 's' : ''}
      ...
      {sessions.length === 0 ? (
        ...
        <div className="space-y-2">
          {sessions.map(session => {
            ...
            const sessionTokens = session.calls.reduce((s, c) => s + (c.total_tokens || c.completion_tokens || 0), 0)
